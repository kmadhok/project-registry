...
- Every new or amended unchecked item must carry `Acceptance:`, `Tests:`,
  `Size:`, `Classes:`, and `Verified-missing:` lines. Size must be `S` or `M`.
  Classes must use only `dependencies`, `ci`, `generated_data`, `public_api`,
  `migrations`, `personal_data`, `plan`, or `contract` (or `none`).
